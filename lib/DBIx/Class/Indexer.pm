...
sub indices {
  ...
  if (@_) {
    ...
    for my $oldkey (keys %$idxs) {
        my $newkey = $class->_table_prefix . $oldkey . $class->_table_suffix;
        $idxs->{$newkey} = delete $idxs->{$oldkey};
    }
...
sub _has_cover_index {
 ...
 #iterate over field strings
 IDXS: for my $flst (@idxs) {
    #iterate over manual indices
    for my $c (@$cols) {
